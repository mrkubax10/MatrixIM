...
void showInfoDialog(char* msg){
    GtkWidget* message=gtk_message_dialog_new(GTK_WINDOW(app->window),GTK_DIALOG_DESTROY_WITH_PARENT,GTK_MESSAGE_INFO,GTK_BUTTONS_OK,msg);
    gtk_dialog_run(GTK_DIALOG(message));
    gtk_widget_destroy(message);
}
int showYesNoDialog(char* msg){
    GtkWidget* dialog=gtk_dialog_new_with_buttons("Question",GTK_WINDOW(app->window),GTK_DIALOG_DESTROY_WITH_PARENT,"Yes",GTK_RESPONSE_YES,"No",GTK_RESPONSE_NO,0);
    GtkWidget* contentArea=gtk_dialog_get_content_area(GTK_DIALOG(dialog));
    GtkWidget* label=gtk_label_new(msg);
    gtk_container_add(GTK_CONTAINER(contentArea),label);
    gtk_widget_show_all(dialog);
